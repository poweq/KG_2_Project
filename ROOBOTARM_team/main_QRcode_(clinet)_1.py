...
class RobotArmPublisher(Node):
    def __init__(self):
        ...
        self.publisher = self.create_publisher(String, 'robot_arm', 10)

    def publish_pose(self, n):
        msg = String()
        msg.data = f"pose {n}"
...
def robot_task():
    ...
    try:
        ...
        robot_arm_node.publish_pose(0)  # pose 0 퍼블리시
        # 작업 시작
     ...
     ####################################### 1단계 #######################################
        ...
        if detect_and_grab_block():
            ...
            robot_arm_node.publish_pose(1)  # pose 1 퍼블리시
        ...
        ####################################### 2단계 #######################################
            ...
            robot_arm_node.publish_pose(2)  # pose 2 퍼블리시
            print("Z축 내리기...")

        ####################################### 3단계 #######################################
            robot_arm_node.publish_pose(3)  # pose 3 퍼블리시
            lower_z()
            if not running or should_exit:
                return  # 실행 중단
            print("블록 배치...")
        ####################################### 4단계 #######################################
            robot_arm_node.publish_pose(4)  # pose 4 퍼블리시
            block_box_match()
            if not running or should_exit:
                return  # 실행 중단
            # time.sleep(2)
            ...
            # print("그리퍼 열기...")
         ###################################### 5단계 #######################################
            robot_arm_node.publish_pose(5)  # pose 5 퍼블리시
